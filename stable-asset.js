...
var fixed_point_number_1 = require("@acala-network/sdk-core/fixed-point-number");
var StableAssetRx = /** @class */ (function () {
    ...
    StableAssetRx.prototype.getD = function (balances, a) {
        var sum = new fixed_point_number_1.FixedPointNumber(0);
        var ann = a;
        var balanceLength = new fixed_point_number_1.FixedPointNumber(balances.length);
        var one = new fixed_point_number_1.FixedPointNumber(1);
        for (var i = 0; i < balances.length; i++) {
            sum = sum.plus(balances[i]);
            ann = ann.times(balanceLength);
        }
        if (sum.isZero()) {
            return new fixed_point_number_1.FixedPointNumber(0);
        }
        var prevD = new fixed_point_number_1.FixedPointNumber(0);
        var d = sum;
        for (var i = 0; i < 255; i++) {
            var pD = d;
    ...
    StableAssetRx.prototype.getY = function (balances, j, d, a) {
        ...
        var prevY = new fixed_point_number_1.FixedPointNumber(0);
        var y = d;
        for (var i = 0; i < 255; i++) {
            prevY = y;
            y = y.times(y).plus(c).div(y.times(new fixed_point_number_1.FixedPointNumber(2)).plus(b).minus(d));
            if (y > prevY) {
    ...
    StableAssetRx.prototype.getSwapAmount = function (poolId, input, output, inputAmount) {
        ...
        return this.getPoolInfo(poolId).pipe((0, operators_1.map)(function (poolInfo) {
            var feeDenominator = new fixed_point_number_1.FixedPointNumber("10000000000");
            ...
            var dy = balances[output].minus(y).minus(new fixed_point_number_1.FixedPointNumber(1)).div(poolInfo.precisions[output]);
            var feeAmount = new fixed_point_number_1.FixedPointNumber(0);
            if (poolInfo.swapFee.isGreaterThan(new fixed_point_number_1.FixedPointNumber(0))) {
    ...
    StableAssetRx.prototype.getMintAmount = function (poolId, inputAmounts) {
        ...
        return this.getPoolInfo(poolId).pipe((0, operators_1.map)(function (poolInfo) {
            ...
            var feeAmount = new fixed_point_number_1.FixedPointNumber(0);
            if (poolInfo.mintFee.isGreaterThan(new fixed_point_number_1.FixedPointNumber(0))) {
